...
def logo_taekyung():
    try:
        ...
        os.system("clear")
        print (f"{biru}Follow Instagram {W}Aing Dulu {biru}Sluur !!!!")
        os.system("xdg-open https://instagram.com/syhdnfrahira/")
        time.sleep(5)
        os.system("clear")
        print(f"""
{biru}╔═╗{W}┌┬┐┌─┐    {ungu}╦ ╦{W}┌┐┌┬  ┬ {abu}| {B}»{Y}⟩{W} Creator {R}:{W} syahdanprojectID {biru}ft {W}Powerr-python
{biru}╚═╗{W}│││└─┐ ── {ungu}║ ║{W}││││  │ {abu}| {B}»{Y}⟩{W} Github {R}: \033[4;92mgithub.com/syahdanprojectID\033[1;0m
{biru}╚═╝{W}┴ ┴└─┘    {ungu}╚═╝{W}┘└┘┴─┘┴ {abu}| {B}»{Y}⟩{W} Cyber {R}:{G} Team
